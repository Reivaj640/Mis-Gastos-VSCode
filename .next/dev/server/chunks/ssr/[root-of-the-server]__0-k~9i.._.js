...
function getAllCategories(settings) {
    const builtIn = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$types$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["CATEGORIES"].map((c)=>({
            value: c.value,
            label: c.label,
            icon: c.icon
        }));
...
function formatCurrencySimple(amount, currencySymbol = "$") {
...
function getExpenseStatus(expense, payments, alertDays, currentPeriod) {
    if (!expense.isActive) return "upcoming";
    const isPaid = payments.some((p)=>p.expenseId === expense.id && p.period === currentPeriod);
...
function getCategoryLabel(category, customCategories) {
    const labels = {
        agua: "Agua",
        luz: "Energía",
        gas: "Gas",
...
function paymentsToCSV(payments, expenses, currencySymbol) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0-k~9i.._.js.map